...
class AuthController extends Controller
{
    public function register(Request $request)
    {
        ...
        */
        $validator = Validator::make($request->all(),[
            ...
            'phone_number' => ['required', 'string', 'unique:users,phone_number','max:13','min:11'],//,'starts_with:[0\+]'],
            'parent_phone_number' => ['required', 'string','max:13','min:11'],//,'starts_with:[0\+]'],
        ...
        $user = User::create([
            ...
            'grade_id'=> Grade::where('name',$request->grade)->get()->id,
            'governorate_id'=> Governorate::where('name',$request->governorate)->get()->id,
        ]);
    ...
    public function login(Request $request){
        ...
        $user = User::where(['email'=>$request->email,'password'=>Hash::make($request->password)])->first();
        if (!$user){
            return apiResponse(false,_('هذا المسخدم غير موجود'),[],401);
        }
        $token = $user->createToken('auth_token')->plainTextToken;
